...
void THREAD_SERVER(void* arg)
{
...
#ifdef MNT_DEBUG
		printf("[SSD_MONITOR] Server Socket Creation error!!! Exiting the Monitor...\n");
        exit(0);
#endif
	}
...
#ifdef MNT_DEBUG
		printf("[SSD_MONITOR] Server Socket Bind Error!!! Exiting the Monitor...\n");
		exit(0);
#endif
	}
...
#ifdef MNT_DEBUG
		printf("[SSD_MONITOR] Server Socket Listen Error!!! Exiting the Monitor...\n");
        exit(0);
#endif
	}
#ifdef MNT_DEBUG
    ...
    if (clientSock == -1) {
#ifdef MNT_DEBUG
        printf("[SSD_MONITOR] Connection failed with Error NO. [%d]\n", errno);
	/*printf("[SSD_MONITOR] Connected to Sock[%d] with ERRNO[%d]\n", clientSock, errno);*/
	/*printf("[SSD_MONITOR] Error No. [%d]\n", errno);*/
#endif	
        exit(0);
    } else {
#ifdef MNT_DEBUG
	printf("[SSD_MONITOR] Connected to Sock[%d] successfully\n", clientSock);
#endif
    }
	g_server_create = 1;

#ifdef MNT_DEBUG
    printf("[SSD_MONITOR] Finally, the Monitor is successfully created and conencted to the client!!!\n");
#endif
}
